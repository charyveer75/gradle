...
class AbstractPmdPluginVersionIntegrationTest extends MultiVersionIntegrationSpec {
    ...
    String requiredSourceCompatibility() {
        ...
        } else if (versionNumber >= VersionNumber.parse('6.0.0') && TestPrecondition.JDK10_OR_LATER.isFulfilled()) {
            "sourceCompatibility = 9"
        } else if (versionNumber >= VersionNumber.parse('6.4.0') && TestPrecondition.JDK11_OR_LATER.isFulfilled()) {
            "sourceCompatibility = 10"
        } else if (versionNumber >= VersionNumber.parse('6.6.0') && TestPrecondition.JDK12_OR_LATER.isFulfilled()) {
            "sourceCompatibility = 11"
        } else if (versionNumber >= VersionNumber.parse('6.13.0') && TestPrecondition.JDK13_OR_LATER.isFulfilled()) {
            "sourceCompatibility = 12"
        } else if (versionNumber >= VersionNumber.parse('6.18.0') && TestPrecondition.JDK14_OR_LATER.isFulfilled()) {
